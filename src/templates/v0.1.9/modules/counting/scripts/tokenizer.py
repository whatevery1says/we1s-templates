...
This script is a clone of the import module's `import_tokenizer.py` script,
except that it has the `Timer` class added to the end.
...
3. Use the WE1S tokenizer to create a `bag_of_words` if the
   user requests it with  `start(mode='we1s')`.
...
To Do: Add the ability to save the spaCy `features` table to the JSON file.
...
class ImportTokenizer:
    ...
    def start(self, bagify_features=False, method=None):
    ...
    def tokenize_doc(self, doc, filename, index, bagify_features=False, method=None):
