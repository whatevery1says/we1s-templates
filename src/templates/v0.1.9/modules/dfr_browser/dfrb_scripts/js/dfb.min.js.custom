...
var view = function() {
    ...
    e.tooltip = c;
    l = function(e, t) {
        e.append("td").classed("weight", true).append("div").classed("proportion", true).style("margin-left", function(e) {
            return d3.format(".1%")(1 - t(e))
        }).append("span").classed("proportion", true).html("&nbsp;")
    };
    e.append_weight_tds = l;
    u = function(e, t) {
        var i;
        if (!VIS.svg) {
            VIS.svg = d3.map()
...
model = function(e) {
    var t = e || {},
        i = {},
        n, o, r, a, d, s, c, l, u, f, p, m, w, _, v, h, g, y, b, x, S, k, I, V, A, j;
    t.ready = {};
...
bib.dfr = function(e) {
    ...
    i.sorting([
        ...
        ["year_journalcontents", "by year, then by publication contents"],
    ...
    n = function(e) {
        var i, n, o, r = e.split("_").filter(function(e) {
                return /\S/.test(e)
            }),
            a = r.length,
            d = "[N/A]";
    ...
    i.citation = function(e) {
        ...
        t = t.replace(/’\./g, ".’");
        t += " <em>" + e.journal + "</em>. ";
        t += d3.time.format.utc("%B %d, %Y")(e.date) + ".";
        t = t.replace(/\.\./g, ".");
        t = t.replace(/_/g, ",");
        t = t.replace(/\t/g, "");
        return t
...
metadata.dfr = function(e) {
    ...
    n = function(e) {
        ...
        t.docs = d3.csv.parseRows(i, function(e, i) {
            ...
            //FIX: a temporary fix for a list view error
            //by Sihwa Park, 07/17/2018
            var d = new Date(e[6].trim())
            if(d == "Invalid Date")
                //d = new Date()
                d = new Date('1900-01-01T00:00:00Z') // Changed by Scott Kleinman on 2019-02-25 to be more conspicuous
            ////////////////////////////////////////////////
            ...
            n = {
                ...
                volume: e[4].trim(),
                issue: e[5].trim(),
                date: d, //FIX: changed from 'new Date(e[6].trim())'
...
view.doc = function(e) {
    ...
    o = t.select("table#doc_topics tbody").selectAll("tr").data(n);
    o.enter().append("tr");
    o.exit().remove();
    o.selectAll("td").remove();
    r = o.append("td").append("a").attr("href", function(e) {
        return view.topic.link(e.topic)
    }).classed("topic_words", true);
    r.append("span").classed("name", true).text(function(t, i) {
        return e.labels[i]
    });
    o.append("td").append("a").attr("href", function(e) {
        return view.topic.link(e.topic)
    }).append("span").classed("words", true).text(function(t, i) {
        return e.words[i].reduce(function(e, t) {
            return e + " " + t.word
        }, "")
    });
    ...
    view.append_weight_tds(o, function(e) {
        return e.weight / i
    });
    o.append("td").classed("td-right", true).text(function(e) {
...
view.model.conditional = function(e) {
    ...
    if (view.dirty("model/conditional")) {
        this.conditional.data = view.model.stacked_series({
            ...
            topics: e.topics,
            streamgraph: e.streamgraph && e.condition_type !== "ordinal"
        });
    ...
    view.model.conditional_plot({
        ...
        data: this.conditional.data[t ? "raw" : "frac"],
        domain_x: this.conditional.data.domain_x,
        domain_y: this.conditional.data[t ? "domain_raw" : "domain_frac"],
...
view.model.list = function(e) {
    var t, i, n, o = d3.sum(e.sums),
        r, a, d, s, c;
    t = d3.select("#model_view_list table tbody").selectAll("tr");
    if (!VIS.ready.model_list) {
        c = utils.clone(VIS.model_view.list.spark);
        c.time.step = VIS.condition.spec;
        d3.select("th#model_view_list_condition a").text(e.type === "time" ? "over time" : "by " + e.condition_name);
        t = t.data(d3.range(e.data.length)).enter().append("tr");
        t.on("click", function(e) {
            view.dfb().set_view(view.topic.hash(e))
        });
        t.classed("hidden_topic", function(t) {
            return e.topic_hidden[t]
        });
        t.append("td").append("a").classed("topic_name", true).attr("href", view.topic.link).text(function(t) {
            return e.labels[t]
        });
        i = t.append("td").append("div").classed("spark", true);
        view.append_svg(i, c).each(function(t) {
        ...
        t.append("td").append("a").classed("topic_words", true).attr("href", view.topic.link);
        n = d3.max(e.sums);
        view.append_weight_tds(t, function(t) {
            return e.sums[t] / n
        });
        t.append("td").text(function(t) {
            return VIS.percent_format(e.sums[t] / o)
        });
        VIS.ready.model_list = true
    }
    t.selectAll("td a.topic_words").text(function(t) {
        return e.words[t].reduce(function(e, t) {
            return e + " " + t.word
        }, "")
    });
    ...
    d = e.sort || VIS.last.model_list.sort || "topic";
    s = e.dir || (d === VIS.last.model_list.sort ? VIS.last.model_list.dir : "up") || "up";
    if (d === "words") {
        r = e.words.map(function(e) {
            return e.reduce(function(e, t) {
                return e + " " + t.word
            }, "")
        })
    } else if (d === "frac") {
        r = e.sums.map(function(e) {
            return -e / o
        })
    } else if (d === "condition") {
        ...
        r = e.labels.map(view.topic.sort_name)
    }
    if (s === "down") {
        a = function(e, t) {
            return d3.descending(r[e], r[t]) || d3.descending(e, t)
        }
    } else {
        a = function(e, t) {
            return d3.ascending(r[e], r[t]) || d3.ascending(e, t)
        }
    }
    VIS.last.model_list.sort = d;
    VIS.last.model_list.dir = s;
    t.sort(a).order();
...
view.topic.words = function(e) {
    ...
    // This section modified by Scott Kleinman to truncate long words and add tooltips
    t.append("td").append("a").attr("class", "tv-top-word-label")
    ...
    .attr("title", function(e) {
        ...
        let length = 14
        return e.word.length > length ? e.word.substring(0, length - 3) + '...' : e.word
    });
    //
    view.append_weight_tds(t, function(t) {
...
view.topic.docs = function(e) {
    ...
    i = d3.select("#topic_docs tbody").selectAll("tr").data(n);
    i.enter().append("tr");
    i.exit().remove();
    i.selectAll("td").remove();
    i.append("td").append("a").html(function(t, i) {
        return e.citations[i]
    });
    i.on("click", function(e) {
        view.dfb().set_view("/doc/" + e.doc)
    });
    view.append_weight_tds(i, function(e) {
        return e.frac
    });
    i.append("td").classed("td-right", true).text(function(e) {
...
view.word = function(e) {
    ...
    h = _.transition().duration(2e3).attr("transform", function(e, t) {
        return "translate(" + l(t) + ",-" + a / 2 + ")"
    }).attr("opacity", 1);
    h.selectAll("text").attr("x", d / (4 * n));
    h.selectAll("rect").attr("width", d / (2 * n)).attr("height", function(e) {
        return f(e.weight)
    });
...
var dfb = function(e) {
    ...
    t.views.set("bib", function(e, i, n) {
        ...
        if (!VIS.ready.bib) {
            VIS.bib_citations = t.m.meta().map(t.bib.citation);
    ...
    d = function() {
        ...
        if (a) {
            ...
            VIS.cur_view = d3.select("div#" + o + "_view");
            ...
            if (VIS.cur_view === undefined) {
                VIS.cur_view = d3.select("div#" + t.default_view[1] + "_view");
                t.views.get("default")()
            }
        }
        if (!view.updating()) {
    ...
    i.load_data = u;
    f = function() {
        u(VIS.files.info, function(e, i) {
            if (typeof i === "string") {
                t.m.info(JSON.parse(i));
                VIS.update(t.m.info().VIS);
                d3.selectAll(".model_title").html(t.m.info().title);
            ...
            u(VIS.files.topic_scaled, function(e, i) {
                if (typeof i === "string") {
